...
function firstStep() { // 提交电话号码，获取［验证码］  
  ...
  if (utils.submitPhoneNum(phoneNum)) {
    hintMsg = null
    return true
  }
  hintMsg = "提交错误，请稍后重试!"
...
function secondStep() { // 提交［验证码］  
  if (!utils.checkIsNotNull(identifyCode)) {
    hintMsg = "请输入验证码!"
    return false
  }

  if (utils.submitIdentifyCode(identifyCode)) {
    hintMsg = null
    return true
  }
  hintMsg = "提交错误，请稍后重试!"
...
function thirdStep() { // 提交［密码］和［重新密码］  
  ...
  if (!utils.isContentEqual(password, rePassword)) {
    hintMsg = "两次密码不一致！"
    return false
  }

  if (utils.submitPassword(password)) {
    hintMsg = "注册成功"
    return true
  }
  hintMsg = "提交错误，请稍后重试!"
